...
// 256 8-bit values; gamma=4
//var gammaTable = [0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x1,0x1,0x1,0x1,0x1,0x1,0x1,0x1,0x1,0x1,0x1,0x1,0x1,0x1,0x1,0x1,0x1,0x2,0x2,0x2,0x2,0x2,0x2,0x2,0x2,0x2,0x2,0x3,0x3,0x3,0x3,0x3,0x3,0x3,0x4,0x4,0x4,0x4,0x4,0x4,0x5,0x5,0x5,0x5,0x5,0x6,0x6,0x6,0x6,0x7,0x7,0x7,0x7,0x8,0x8,0x8,0x9,0x9,0x9,0xa,0xa,0xa,0xb,0xb,0xc,0xc,0xc,0xd,0xd,0xe,0xe,0xe,0xf,0xf,]
...
function gammaRgb48(gammaTable, rgb48) {
  if (gammaTable != null)
...
function getRgb48Bytes(depth, rgb48) {
  bytes = []
  if (depth == 48) {
    bytes.push(rgb48[0] & 0xFF);
    bytes.push(rgb48[0] >> 8);
    bytes.push(rgb48[1] & 0xFF);
    bytes.push(rgb48[1] >> 8);
    bytes.push(rgb48[2] & 0xFF);
    bytes.push(rgb48[2] >> 8);
  } else if (depth == 24) {
    bytes.push(rgb48[0] >> 8);
    bytes.push(rgb48[1] >> 8);
    bytes.push(rgb48[2] >> 8);
...
LEDs.prototype.packData = function() {
  ...
  if (this.socket != null) {
    ...
    for (var y = 0; y < this.height; y++) {
      for (var x = 0; x < this.width; x++) {
        var rgb48 = gammaRgb48(this.gammaTable, this.rgbs48[x][y]);
        packet[dest++] = rgb48[0];
        packet[dest++] = rgb48[1];
        packet[dest++] = rgb48[2];
      }
    ...
    for (var y=0; y<this.height; y++) {
      if ((this.layoutLeftToRight && y % 2 == 0) ||
        ...
        // flip
        for (var x=0; x<this.width; x++) {
          var rgb48 = gammaRgb48(this.gammaTable, this.rgbs48[this.width-1-x][y]);
          var bytes = getRgb48Bytes(this.depth, rgb48);
        ...
        for (var x=0; x<this.width; x++) {
          var rgb48 = gammaRgb48(this.gammaTable, this.rgbs48[x][y]);
          // console.log(rgb48 + " " + this.rgbs48[x][y]);
          var bytes = getRgb48Bytes(this.depth, rgb48);
          //console.log(rgb48 + ": " + bytes);
          for (i in bytes)
            packet.push(bytes[i]);
