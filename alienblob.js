var AlienBlobC = require("./build/Release/alienblob.node").AlienBlob;

function AlienBlobDrawer(width, height, numColors) {
    this.name = "AlienBlob";
    this.pos = Math.random() * 1e3;
    this.colorIndex = 0;
    this.values = {speed: 3, colorSpeed: 0, detail: 3, zoom: 70, 
		   audioSensitivity: 10, maxAudioShift: 10};
    this.ranges = {speed: [0,100], colorSpeed: [0,50], detail: [1,4], zoom: [0,100], 
		   audioSensitivity: [0,100], maxAudioShift: [0,100]};
    this.decay = 0.5;
    this.speedMultiplier = 0.07;
    this.audioLevel = 0;
    
    this.alienblob = new AlienBlobC(width, height, numColors);
    this.indices = new Int32Array(width*height);
}
...
AlienBlobDrawer.prototype.draw = function(leds, palette) {
    this.alienblob.run(this.pos, this.values["detail"], this.decay, this.values["zoom"]/100.0); // C
    //this.alienblob.run(this.pos, this.values["zoom"]/100, this.values.detail, this.decay);
